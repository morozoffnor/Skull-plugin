...
public class Main extends JavaPlugin{
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (label.equalsIgnoreCase("skull")) {
			...
			if (args.length == 0) {
				// give their head
				player.sendMessage(ChatColor.translateAlternateColorCodes('&', "&6You've been given the skull of &c" + player.getName()));
				player.getInventory().addItem(getPlayerHead(player.getName()));
				return true;
			}
			// give a specific player skull
			player.sendMessage(ChatColor.translateAlternateColorCodes('&', "&6You've been given the skull of &c" + args[0]));
